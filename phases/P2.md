...
3. Output Formatting
   ...
   ```bash
   find . -type f | xargs bydefault validate
   local/app.conf ✓
   metadata/default.meta ✓
   metadata/local.meta ✓
   README ✗
   • Line 0: Invalid file extension - must be .conf or .meta
   ...
   ```bash
   find . -type f | xargs bydefault validate --verbose
   Validating local/app.conf...
...
3. Command Options
   ...
   - [x] Implement --report option for structured validation summary
   - [ ] Add tests for each command option
